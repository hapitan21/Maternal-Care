...
export function normalizeAdminSystemSettings(data, fallback) {
  ...
  return {
    ...
    clinicOpeningTime: normalizeTime(row.clinic_opening_time, fallback.clinicOpeningTime),
    clinicClosingTime: normalizeTime(row.clinic_closing_time, fallback.clinicClosingTime),
    appointmentDuration: String(row.default_appointment_duration_minutes ?? fallback.appointmentDuration),
    bookingInterval: String(row.booking_interval_minutes ?? fallback.bookingInterval),
    cancellationWindow: String(row.cancellation_window_hours ?? fallback.cancellationWindow),
    ...
    dashboardRefreshInterval: String(row.dashboard_refresh_seconds ?? fallback.dashboardRefreshInterval),
  };
}

export function hasSettingsSectionChanges(values, baseline, section) {
  return (sectionKeys[section] || []).some((key) => values[key] !== baseline[key]);
}
...
export function validateSettingsSection(section, values) {
  if (section === "general") {
    ...
    if (systemName.length > 160) return "System Name cannot exceed 160 characters.";
    if (!clinicName) return "Clinic Name is required.";
    if (clinicName.length > 160) return "Clinic Name cannot exceed 160 characters.";
    if (email && !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(email)) return "Enter a valid Clinic Email.";
    if (email.length > 254) return "Clinic Email cannot exceed 254 characters.";
    if (values.contactNumber.trim().length > 50) return "Contact Number cannot exceed 50 characters.";
    if (values.clinicAddress.trim().length > 500) return "Clinic Address cannot exceed 500 characters.";
    if (!values.timezone.trim()) return "Timezone is required.";
    if (values.timezone.trim().length > 80) return "Timezone cannot exceed 80 characters.";
    return "";
  }

  if (section === "appointments") {
    if (!values.clinicOpeningTime || !values.clinicClosingTime) return "Clinic opening and closing times are required.";
    if (values.clinicClosingTime <= values.clinicOpeningTime) return "Clinic Closing Time must be later than Clinic Opening Time.";
    return validateInteger(values.appointmentDuration, 15, 240, "Default Appointment Duration")
      || validateInteger(values.bookingInterval, 5, 240, "Booking Interval")
      || validateInteger(values.cancellationWindow, 0, 168, "Cancellation Window")
      || validateInteger(values.maximumDailyAppointments, 1, 500, "Maximum Daily Appointments");
  }
  ...
  if (section === "notifications") {
    ...
    if (!primaryReminder || primaryReminder < 1 || primaryReminder > 168) return "Select a valid Appointment Reminder Timing.";
    if (secondReminder !== null && (secondReminder < 1 || secondReminder > 168 || secondReminder >= primaryReminder)) {
...
export async function updateAdminSystemSettings(section, values) {
  ...
  if (error) {
    ...
    return { settings: null, error: "Unable to save settings. Please try again." };
  }
